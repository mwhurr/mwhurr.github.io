...
function canada(userAnswer) {
  ...
  // 100 pts
  const isCorrect = userAnswer.toUpperCase() === 'OTTOWA';
...
const isCorrect = upper ===   userAnswer;
...
//word.substr(0,3) works too
const isCorrect = a === userAnswer;
...
function squared(userAnswer, number) {
  ...
  //converts string into number for use with ===
...
function larger(userAnswer, num1, num2) {
  ...
  // 300 pts
  //console.log(userAnswer, num1, num2)

const isCorrect = (userAnswer >= num1)&&(userAnswer>= num2);
    if (isCorrect) {
    points += 300; 
  }
  return isCorrect;
